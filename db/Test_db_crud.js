 ...
 */
connectDB = function()  {
	const dbConnection_Test_db = mongoose.createConnection('mongodb://' + properties.Test_db_dbUrl, function(err){
		if(err) {
			logger.error(err);
			setTimeout(function() {
		...
		{
		    exports.connection = dbConnection_Test_db;
			logger.info("MongoDB connected at: " + properties.Test_db_dbUrl);
			
			//START IMPORT RESOURCE

			require('./Test_db_schema.js');

			require('../resource/Test_db/Annonce.js');
			require('../resource/Test_db/User.js');
